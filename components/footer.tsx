...
const socialLinks = [
  ...
  {
    name: "X",
    href: `https://x.com/${conf.X_USERNAME}`,
    icon: (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
  ...
  {
    name: "LinkedIn",
    href: `https://linkedin.com/in/${conf.AUTHOR.NAME}`,
    icon: (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="container mx-auto max-w-screen-lg px-6 py-12">
        ...
        <div className="mt-10 pt-6 border-t border-border/40 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-foreground">
            © {new Date().getFullYear()} {conf.AUTHOR.NAME}. All rights
            reserved.
          </p>
          ...
          <p className="text-xs text-muted-foreground">
            Built with <span className="inline-block animate-pulse">❤️</span>{" "}
            using Next.js
          </p>
        </div>
      </div>
